...
app.delete("/books/:id", async (req, res, next) => {
    try {
        ...
        res.status(201).json({ message: "Tutti i libri rimossi" });
        } else {
        const book = await Book.findByIdAndDelete(req.params.id);
        res.status(201).json({ message: "Libro rimosso" });
        }
    } catch (err) {
        res.status(400).json({ message: err.message });
    }
...
app.put("/books/:id", async (req, res, next) => {
    try {
        ...
        if (!book) {
            re.status(404).json({ message: "Libro non trovato!" });
        } else {
        ...
        if (title && author && year) {
            ...
            await book.save();
            res.status(201).json({ message: "Libro modificato correttamente" });
            }
        }
    } catch (err) {
        res.status(400).json({ message: err.message });
